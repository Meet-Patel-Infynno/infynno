...
const Forgot = ({ setCheckEmail }) => {
  const { isLoading, error } = useSelector((state) => ({
    isLoading: state.authentication.userData.isLoading,
    error: state.authentication.userData.form.error,
  }));
  ...
  useEffect(() => {
    if (error !== null) {
      !isLoading && !error && setCheckEmail(true);
    }
  }, [isLoading, error]);
  ...
  // useEffect(() => {
  //   error ? setCheckEmail(false) : setCheckEmail(true);
  // }, [error]);
